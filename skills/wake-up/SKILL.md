...
1. **Load memories.** Call `profile` from the memory MCP server.
   Use whatever topic context is available — the user's first
   message, the hook's session description, or a general term
...
2. **Announce.** Call `announce` on goclauded with your pool name
   ...
   short topic. If the user hasn't stated a topic yet, announce
...
3. **Scan the index tier.** Look at the memories that came back
   ...
   the session context or the user's first message, load them
...
5. **Report.** Give the user a brief orientation:
   - Who they are and what you remember (1-2 sentences from
     the always-tier user/feedback memories)
