...
- **Constants in `constant/` packages** - constants live in the subsystem's `constant/` package (e.g. `pkg/tool/gobuild/constant/`), in `constant.go`. Large non-vocabulary assets (CSS, JS, SQL, HTML string blobs) go to domain files beside it in the same package - `style.go`, `script.go`, `query.go` - keeping `constant.go` readable as the vocabulary home. Don't create package-local `constant.go` files - add to the nearest `constant/` package and export the constant, even when only one package uses it today. Deeper subpackages share their subsystem root's `constant/` rather than growing their own. Group with `const (...)` blocks per semantic domain - multiple groups with comment headers are the idiom for large vocabularies. When an iota enum is defined with a named type, both the type and the const values live in `constant.go` beside each other - separating them across packages creates circular imports.
