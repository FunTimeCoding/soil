...
func Dialog() {
	...
	if true {
		...
		callback := locator.New(constant.Localhost).Port(
			constant.ListenPort,
		).Insecure().Path(constant.LocationCallback).String()
	...
	if false {
		m.MustOpenDialog(
			model.OpenDialogRequest{
				...
				URL: locator.New(constant.Localhost).Port(
					constant.ListenPort,
				).Insecure().Path(constant.LocationDialog).String(),
				Dialog: model.Dialog{
