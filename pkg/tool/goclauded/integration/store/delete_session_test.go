...
func TestDeleteSessionCascadesChildRecords(t *testing.T) {
	...
	_, f := s.Store.SetLabel("session-1", "role", "reviewer")
	...
	labels, g := s.Store.LabelsBySession("session-1")
	assert.FatalOnError(t, g)
	assert.Count(t, 1, labels)
	pulses, h := s.Store.PulsesBySession("session-1")
	assert.FatalOnError(t, h)
	assert.Count(t, 1, pulses)
	eventCount, i := s.Store.CountEvents()
