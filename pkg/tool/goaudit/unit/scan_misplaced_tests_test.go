...
func TestMisplacedTestsFlagged(t *testing.T) {
	...
	assert.String(t, "misplaced_test", result[0].Key)
	assert.String(t, "pkg/alfa/parse_test.go", result[0].Path)
}

func TestMisplacedTestsHomesPass(t *testing.T) {
	v := virtual_file_system.New()
	v.WriteString("pkg/alfa/unit/parse_test.go", "package unit\n")
	v.WriteString(
		"pkg/alfa/integration/client/client_test.go",
		"package client\n",
	)
	v.WriteString("pkg/tool/gotestd/unit/option_test.go", "package unit\n")
	v.WriteString(
		"pkg/tool/gotestd/integration/worker/worker_test.go",
		"package worker\n",
	)
	v.WriteString("internal/bravo/unit/bravo_test.go", "package unit\n")
	v.WriteString("charlie/unit/charlie_test.go", "package unit\n")
...
func TestMisplacedTestsNestedHomeFlagged(t *testing.T) {
	v := virtual_file_system.New()
	v.WriteString(
		"pkg/alfa/subsystem/integration/client/client_test.go",
		"package client\n",
	)
	result := scan.MisplacedTests(v)
	assert.Integer(t, 1, len(result))
