...
func testInstance(t *testing.T) *Instance {
	...
	return NewInstance(identity.New("gotest", "test tool", "gotest"))
}
...
func TestWebDefaults(t *testing.T) {
	...
	assert.Integer(t, constant.ListenPort, a.GetInteger(Port))
	assert.String(t, constant.Loopback, a.GetString(BindAddress))
...
func TestWebEnvironmentOverridesDefault(t *testing.T) {
	...
	a.Web()
	assert.Nil(t, a.ParseArguments(nil))
	assert.Integer(t, 9000, a.GetInteger(Port))
	assert.String(t, "0.0.0.0", a.GetString(BindAddress))
